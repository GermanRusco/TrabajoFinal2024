...
/* TODO Include header files from BT configurator */
...
int main(void)
{
    ...
    /*INICIALIZACION DE LA PLACA-BSP */
    result = cybsp_init() ;
    ...
    /*INCIALIZACION DE UART*/
    cy_retarget_io_init(CYBSP_DEBUG_UART_TX, CYBSP_DEBUG_UART_RX,CY_RETARGET_IO_BAUDRATE);

    /*INCIALIZACION LED USUARIO*/
    cyhal_gpio_init(CYBSP_USER_LED,CYHAL_GPIO_DIR_OUTPUT, CYHAL_GPIO_DRIVE_STRONG, CYBSP_LED_STATE_OFF);

    /* CONFIGURACION LED DE CONEXION A BLUETOOTH */
    cyhal_pwm_init(&pwm_obj, CYBSP_USER_LED2, NULL);
    cyhal_pwm_set_duty_cycle(&pwm_obj, LED_OFF_DUTY, 2);

    /*INICIALIZACION LED DE CONEXION A BLUETOOTH*/
    cyhal_pwm_start(&pwm_obj);

    /*CONFIGURACION SERVOMOTOR*/
    result=cyhal_pwm_init(&status_pwm_obj, P10_0, NULL);
    if (result != CY_RSLT_SUCCESS)
    {
        CY_ASSERT(0);
    }

    /*INCIALIZACION SERVOMOTOR*/
    cyhal_pwm_start(&status_pwm_obj);

    /*CONFIGURACION DECODIFICADOR DE CUADRATURA*/
    result = cyhal_quaddec_init(&quaddec_obj, P9_0, P9_1, NC, CYHAL_QUADDEC_RESOLUTION_2X, NULL, 100000);
    if (result != CY_RSLT_SUCCESS)
    {
         CY_ASSERT(0);
    }

    /*INICIALIZACION DECODIFICADOR DE CUADRATURA*/
    result = cyhal_quaddec_start (&quaddec_obj);
    CY_ASSERT( CY_RSLT_SUCCESS == result);

    printf("**********Application Start*****************\n");

    /* TODO Configure platform specific settings for the BT device */
    cybt_platform_config_init(&cybsp_bt_platform_cfg);

    /* TODO Initialize stack and register the callback function */
            ...
            if(p_event_data->ble_advert_state_changed == BTM_BLE_ADVERT_OFF ) /* Advertising stopped */
            			{
            				...
            				{
                        	    cyhal_pwm_set_duty_cycle(&pwm_obj, LED_ON_DUTY, 2);

            				}
            			}
                        else /* Advertising is on - LED blink */
                        {
                            cyhal_pwm_set_duty_cycle(&pwm_obj, LED_BLINK_DUTY, 2);
                        }
            result = WICED_BT_SUCCESS;
			break;
     ...
     for (int i = 0; i < app_gatt_db_ext_attr_tbl_size; i++)
     {
         ...
         if (app_gatt_db_ext_attr_tbl[i].handle == p_write_req->handle)
         {
             ...
             if (app_gatt_db_ext_attr_tbl[i].max_len >= p_write_req->val_len)
             {
                 ...
                 {
                // TODO Add action when specified handle is written
                 case HDLC_PSOC_LED_VALUE:
                 cyhal_gpio_write(CYBSP_USER_LED, app_psoc_led[0] == 0 );
                 printf( "Turn the LED %s\r\n", app_psoc_led[0] ? "ON" : "OFF" );

                 /*CALIBRACION DEL SERVOMOTOR A 0°*/
                 cyhal_pwm_set_duty_cycle(&status_pwm_obj, 11, 50);
                 cyhal_system_delay_ms(500);
                 cyhal_pwm_set_duty_cycle(&status_pwm_obj, 1.5, 50);
                 cyhal_system_delay_ms(500);
                 angle=1.5;

                 /*ESTRUSTURA PARA DECODIFICADOR DE CUADRATURA*/
                 while (1)
                           {

                	           //LECTURA ACTUAL DEL DECODIFICADOR DE CUADRATURA
                               count = cyhal_quaddec_read_counter(&quaddec_obj);

                               //COMPROBACION DE DIRECION DE ROTACION DEL ENCODER
                               if (count > prev_count)
                               {

                            	//GIRO EN SENTIDO HORARIO
                               	angle=angle-a;
                               	cyhal_pwm_set_duty_cycle(&status_pwm_obj, angle, 50);
                               ...
                               //REEMPLAZO VALOR ACTUAL DE LA ROTACION DEL ENCODER
                               prev_count = count;
